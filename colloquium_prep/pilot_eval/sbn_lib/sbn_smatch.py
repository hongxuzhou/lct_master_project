...
def ill_form_checker(sbn):
    try:
        SBNGraph().from_string(sbn, is_single_line=True).to_penman_string()
        return True
    except Exception as e:
        print(e)
        return False

# Legacy smatch is unused by the smatchpp toolkit (scoring goes through smatchpp).
# Guarded so this module imports cleanly without smatch.py/amr.py present on HPC.
try:
    from smatch import score_amr_pairs
except ImportError:
    score_amr_pairs = None
