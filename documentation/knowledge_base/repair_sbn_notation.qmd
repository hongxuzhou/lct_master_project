...
**C3 — `CONJUNCTION` boundaries are permeable, `CORRECTION` boundaries are not.** A positive index representing a thematic role shall **never cross into** a `CORRECTION` box. A negative index inside a `CORRECTION` box is permitted to cross out as long as it targets an ancestor box (e.g., `BOX 0`). However, indices shall never cross laterally between sibling boxes (e.g., from a `CORRECTION` box into a `CONJUNCTION` box). See the repair toolkit below for how to resolve illegal inbound or lateral links.

**C4 — Under certain conditions, the content nested in a`CORRECTION` box can be referred to.** However, discourse referents inside a CORRECTION box survive and remain anaphorically accessible. Quarantining a reparandum removes what the speaker commits to propositionally, but it does not remove the uttered entities from the discourse referent domain. According to SBN, entities like proper nouns or definites naturally project to BOX 0. For self-repairs, we extend this: entities introduced in a reparandum project their referential availability to the enclosing context. Therefore, anaphoric indices (like ANA) referencing these entities do not technically violate the structural barrier of C3, as they pick up the projected referent. This is where `CORRECTION` genuinely differs from `NEGATION`, and it is a positive property of the relation, not a loophole.  
...
go.v.01 Theme -2 Time -1                   % go to,
    CONJUNCTION <2                         % well,
time.n.08 TPR now                          % [went]
go.v.01 Theme -4 Time -1 Destination +1    % went to
church.n.02                                % the church
```

![DAG: Time & aspect replacement - "She will go to, well, went to the church"](images/time_aspect_replace.png)

This case demonstrates device ①. The speaker abandons the phrase *will go to* before uttering the destination. Therefore, the first predicate (go.v.01) takes no Destination role.
This example illustrates why tense and aspect self-repairs do not suffer from the topological limitations that affect separators. In SBN, tense and aspect are not topological separators but ordinary concepts (time.n.08) and edge labels (Time). Consequently, a tense/aspect self-repair is structurally identical to a standard verb self-repair. The initial event concept and its associated time node are smoothly quarantined inside the `CORRECTION` box, while the corrected event and its new time node are introduced safely in the `CONJUNCTION` box. All indices remain perfectly legal and natural.
...
## Topological Limitations: The "Separator vs. Concept" Divide
The `CORRECTION` mechanism proposed in this notation is designed to quarantine node-level Concepts and their argument edges. Therefore, it is mathematically and structurally impossible to encapsulate a topological Separator inside a concept-level `CORRECTION` box. Attempting to do so would destroy the validity of the underlying Directed Acyclic Graph (DAG). This architectural divide restricts the notation from handling repairs that involve logical operators or discourse structures.

### Quantifier Replacement
"Every, no, some dishes are tasty"
Reason:  In SBN, universal quantification (e.g., "every") is not represented as a single concept node, but is constructed using double `NEGATION` separators that open new nested context boxes. When the speaker repairs the quantifier to "some" (an existential quantifier that does not require new boxes), it is impossible to use a `CORRECTION` box to retrospectively "undo" or quarantine the topological boundaries of the already-opened NEGATION boxes.

### Rhetorical Relation Replacement
"I helped my mum when, even though I was busy."

Reason: Rhetorical relations (such as `EXPLANATION`, `CONTRAST`, or `CONCESSION`) act as Separators that connect two distinct discourse contexts. They cannot be quarantined inside a `CORRECTION` box that is meant for semantic concepts.

## Modal Verb Replacement
"I think you would, well, should work harder"

Reason: Similar to quantifiers and rhetorical relations, modal verbs (like "should" or "would") are frequently treated as structural separators (e.g., `NECESSITY` or `POSSIBILITY`) in formal semantics. For the same reason above, they are immune to the current CORRECTION boxing mechanism.

## Linear Serialisation Limitations: Discontinuous (Delayed) Repair
